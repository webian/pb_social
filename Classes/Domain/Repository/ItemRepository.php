                    ...
                    if($feeds && $feeds->count() > 0){
                        ...
                        if($devMod || ($feed->getDate()->getTimestamp() + $refreshTimeInMin*60) < time()){
                            try{
                                ...
                                if(property_exists(json_decode($curl_response),"error")){ throw new \Exception($curl_response); }
                        ...
                        if($devMod || ($feed->getDate()->getTimestamp() + $refreshTimeInMin*60) < time()){
                            try{
                                ...
                                if($blogName == "MYDASHBOARD"){
                                    ...
                                    $feed->setResult(json_encode($tumblr->getBlogPosts($blogName,array('limit' => $feedRequestLimit))));
                                }
                                $this->update($feed);
                            }
                            catch (\Exception $e){
